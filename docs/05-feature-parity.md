...
| **Hardlinks** | **done** | Bump two blocks together to secure their connection: drawn red, immune to distance, undone by bumping again, gone when either leaves the table. Purely about objects, so it works the same on the screen and under the camera |
| **Rotation triggers the envelope** | **done** | An oscillator with an *envelope* puck beside it sounds when it arrives and re-articulates on every new note you turn to, which is the original's built-in envelope with ours as a separate object. Turning a cube is the one thing a camera reads perfectly |
| **Tonality edited note by note** | **done** | Twelve fields around the *key* block, tapped in and out, override the scale presets for the whole table. The same rim-tap pattern the sequencer's sixteen pads already use |
| Locking a finger parameter | **out of scope** | The original locked the touch parameter because a hand sweeping a multi-touch table knocks the value it just set. We have a camera over a screen, not a surface hands rest on, so we do not have that problem, and a slider that silently ignores you is a hidden mode not worth its cost |
| Hold a finger on a connection to mute it | **out of scope** | We already swipe to cut and tap to mend, which is visible, deliberate and reliable through hand tracking. A third gesture on the same target would add an accidental-mute failure mode to buy a momentary variant |
...
| The music of the spheres | **done** | An orrery with Kepler's periods and real moon counts, Holst-styled sounds per world, God-mode n-body gravity with merging and shattering, rockets, starships and visitors |
