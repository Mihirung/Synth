...
## Three gestures from the original table

These are not ours. They come from the Reactable Live! manual, and they are things you do
to the table rather than to a particular block. [docs/02](02-how-it-worked.md) quotes the
manual, including the gestures we read and deliberately did not take.

The filter for what to take is the camera. The original table watched fingers on its own
glass, so much of its vocabulary is about where a hand touches the surface. Ours watches
printed markers from above and tracks hands in the air. Gestures made of **objects** cross
that gap perfectly; gestures made of **fingers on glass** mostly do not, and where we
already had an equivalent we kept ours.
...
- **Turn an oscillator to play it.** On the original, rotating an oscillator set its pitch
  *and* triggered its envelope, and the envelope could be flat, in which case nothing
  ...
  its own drones, but put an **envelope** puck beside it and it becomes something you play.
  It sounds once when the envelope arrives, and again on every new note you turn to; the
  envelope's sustain decides what happens in between, so PAD holds and PLUCK dies away and
  waits for you. Take the envelope away and it goes back to droning. Turning a cube is the
  one gesture a camera reads perfectly, which is why this one is worth having.
...
turn to re-articulates, and placing either of them sounds it once: the original triggered its
built-in envelope on rotation, and this is that, with the envelope as an object you can take
away. The envelope's sustain decides whether it holds between turns. Without one the
oscillator drones as it always did.
